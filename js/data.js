// ============================================================
//   MathReview — Data Layer
//   Sample content: 高等数学 + 线性代数
//   ============================================================
...
const MATH_DATA = {
  ...
  formulas: [
    ...
    },
    {
      ...
      chapter: '一元微分学',
      title: '泰勒公式（带拉格朗日余项）',
      latex: 'f(x) = f(x_0) + f\\'(x_0)(x-x_0) + \\frac{f^{\\prime\\prime}(x_0)}{2!}(x-x_0)^2 + \\cdots + \\frac{f^{(n)}(x_0)}{n!}(x-x_0)^n + \\frac{f^{(n+1)}(\\xi)}{(n+1)!}(x-x_0)^{n+1}',
...
const Storage = {
  ...
  set(key, value) {
    try { localStorage.setItem('mathreview:' + key, JSON.stringify(value)); } catch {}
  }
};
...
function getMergedData() {
  ...
  data.formulas.forEach(f => { if (bookmarks[f.id]) f.bookmarked = true; });
  const probMarks = Storage.get('problemBookmarks', {});
  data.problems.forEach(p => { if (probMarks[p.id]) p.bookmarked = true; });
  return data;
}
