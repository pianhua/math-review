...
/**
 * Generate similar problems based on user's weak problems
 */
async function generateWeakPointReview(weakProblemList, reviewCount = 5) {
  if (!API_KEY) {
    throw new Error('MINIMAX_API_KEY not configured');
  }

  const prompt = `你是一位考研数学出题专家。请基于以下学生的薄弱题目，生成 ${reviewCount} 道**类似但不同**的新题，帮助学生巩固薄弱环节。
...
3. 每道题的 type 字段必须是 "选择题"
4. 保持相同的章节分布和知识点
5. 难度相当或略低（帮助巩固），题目内容必须不同（避免重复）
6. 考察相同的知识点，但用不同的数字、函数或场景
  ...
  "problems": [
    {
      ...
      "correct": "C",
      "knowledge_point": "本题涉及的核心知识点（1-2个公式或定理，LaTeX格式）",
      "explanation": "详细解析（LaTeX格式）"
    }
  ]
}
  ...
  try {
    const response = await fetch(`${API_BASE}/text/chatcompletion_v2`, {
      ...
      body: JSON.stringify({
        model: 'MiniMax-M2.7',
        messages: [
          { role: 'system', content: '你是一位专业的考研数学出题专家，擅长根据学生的薄弱题目生成类似的巩固练习。' },
          { role: 'user', content: prompt }
        ],
        stream: false
      })
    ...
    console.error('Generate weak point review failed:', error);
    throw error;
  }
}

module.exports = { generateDailyReview, generateWeakPointReview };
