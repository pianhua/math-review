...
const MATH_DATA = {
  formulas: [
    {
      id: 'f-001', chapter: '极限与连续', title: '两个重要极限',
    ...
    {
      id: 'f-002', chapter: '极限与连续', title: '等价无穷小（$x \\to 0$）',
      latex: '\\begin{aligned}\\sin x &\\sim x, & \\tan x &\\sim x, & 1 - \\cos x &\\sim \\frac{x^2}{2} \\\ln(1+x) &\\sim x, & e^x - 1 &\\sim x, & (1+x)^\\alpha - 1 &\\sim \\alpha x\\end{aligned}',
      description: '等价无穷小替换是求极限的核心技巧，注意仅在乘除因子中可替换，加减项不能直接替换。'
    },
    {
      id: 'f-003', chapter: '一元微分学', title: '导数定义',
    ...
    {
      id: 'f-004', chapter: '一元微分学', title: '泰勒公式（带拉格朗日余项）',
    ...
    {
      id: 'f-005', chapter: '一元微分学', title: '洛必达法则',
    ...
    {
      id: 'f-006', chapter: '矩阵与行列式', title: '行列式按行展开',
    ...
    {
      id: 'f-007', chapter: '矩阵与行列式', title: '矩阵乘法',
    ...
    {
      id: 'f-008', chapter: '矩阵与行列式', title: '逆矩阵公式（2阶）',
  ...
  problems: [
    {
      id: 'p-001', chapter: '极限与连续', source: '教材例题', difficulty: 'basic', type: '计算题',
      content: '求极限 $\\displaystyle \\lim_{x \\to 0} \\frac{\\tan x - \\sin x}{x^3}$',
      options: [{ label: 'A', text: '$\\frac{1}{2}$' }, { label: 'B', text: '$\\frac{1}{6}$' }, { label: 'C', text: '$\\frac{1}{3}$' }, { label: 'D', text: '$0$' }],
      correct: 'A',
      explanation: '$$\\begin{aligned}\\tan x - \\sin x &= \\frac{\\sin x}{\\cos x} - \\sin x = \\sin x \\cdot \\frac{1 - \\cos x}{\\cos x} \&\\sim x \\cdot \\frac{x^2/2}{1} = \\frac{x^3}{2}\\end{aligned}$$因此原式 $= \\frac{1}{2}$。'
    },
    {
      id: 'p-002', chapter: '极限与连续', source: '2021年真题', difficulty: 'exam', type: '选择题',
    ...
    {
      id: 'p-003', chapter: '一元微分学', source: '教材例题', difficulty: 'basic', type: '计算题',
    ...
    {
      id: 'p-004', chapter: '一元微分学', source: '张宇1000题', difficulty: 'advanced', type: '证明题',
    ...
    {
      id: 'p-005', chapter: '矩阵与行列式', source: '2022年真题', difficulty: 'exam', type: '选择题',
    ...
    {
      id: 'p-006', chapter: '矩阵与行列式', source: '教材例题', difficulty: 'basic', type: '计算题',
...
async function seed() {
  ...
  const existingFormulas = query('SELECT COUNT(*) as count FROM formulas');
  if (existingFormulas[0].count === 0) {
    for (const f of MATH_DATA.formulas) {
      run(
        'INSERT INTO formulas (id, chapter, title, latex, description) VALUES (?, ?, ?, ?, ?)',
        [f.id, f.chapter, f.title, f.latex, f.description]
      );
    }
    console.log(`Seeded ${MATH_DATA.formulas.length} formulas`);
  }

  const existingProblems = query('SELECT COUNT(*) as count FROM problems');
  if (existingProblems[0].count === 0) {
    for (const p of MATH_DATA.problems) {
      run(
        'INSERT INTO problems (id, chapter, source, difficulty, type, content, options, correct, explanation) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [p.id, p.chapter, p.source, p.difficulty, p.type, p.content, JSON.stringify(p.options), p.correct, p.explanation]
      );
    }
    console.log(`Seeded ${MATH_DATA.problems.length} problems`);
  }
